...
/** @param mixed $row @return array<string, mixed> */
function normalizeInventoryMovement($row): array { $record = is_array($row) ? $row : []; return ['id' => trim((string) ($record['id'] ?? '')), 'movement_type' => trim((string) ($record['movement_type'] ?? '')), 'reason_code' => trim((string) ($record['reason_code'] ?? '')), 'quantity_before' => isset($record['quantity_before']) ? (int) $record['quantity_before'] : null, 'quantity_after' => (int) ($record['quantity_after'] ?? 0), 'quantity_delta' => isset($record['quantity_delta']) ? (int) $record['quantity_delta'] : null, 'note' => isset($record['note']) && trim((string) $record['note']) !== '' ? trim((string) $record['note']) : null, 'created_at' => trim((string) ($record['created_at'] ?? ''))]; }
