...
if ($bootstrapPath === null) {
    forge_staff_send_fallback_response(
        ...
        [
            ...
            'error' => [
                'code' => 'server_error',
                'message' => 'Order completion is currently unavailable.',
            ],
        ]
    );
    exit;
...
try {
    ...
    $result = $repository->completeOrder($forgeOrderUuid);
...
} catch (\Forge\Server\CompleteOrderNotAllowedException $exception) {
    \Forge\Server\ApiResponse::send(
        409,
        \Forge\Server\ApiResponse::error('order_not_completable', $exception->getMessage() !== '' ? $exception->getMessage() : 'That order cannot be completed right now.')
    );
} catch (\InvalidArgumentException $exception) {
    \Forge\Server\ApiResponse::send(
        422,
        \Forge\Server\ApiResponse::error('invalid_request', $exception->getMessage() !== '' ? $exception->getMessage() : 'The order completion request is invalid.')
    );
    ...
    forge_staff_log_unexpected_exception($exception, $bootstrapPath, 'staff complete order endpoint');
    forge_staff_send_fallback_response(
        500,
        [
            'application' => 'Forge',
