...
try {
    ...
    $result = $repository->cancelOrder($forgeOrderUuid);
...
} catch (\Forge\Server\CancelOrderNotAllowedException $exception) {
    \Forge\Server\ApiResponse::send(
        409,
        \Forge\Server\ApiResponse::error('order_not_cancellable', $exception->getMessage() !== '' ? $exception->getMessage() : 'That order cannot be cancelled.')
    );
} catch (\InvalidArgumentException $exception) {
    \Forge\Server\ApiResponse::send(
        422,
        \Forge\Server\ApiResponse::error('invalid_request', $exception->getMessage() !== '' ? $exception->getMessage() : 'The cancellation request is invalid.')
    );
    ...
    forge_staff_log_unexpected_exception($exception, $bootstrapPath, 'staff cancel order endpoint');
    forge_staff_send_fallback_response(
        500,
        [
            'application' => 'Forge',
