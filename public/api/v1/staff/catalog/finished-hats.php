...
try {
    ...
    if ($method === 'GET') {
        \Forge\Server\ApiResponse::send(
            ...
            \Forge\Server\ApiResponse::success([
                'finished_hats' => $repository->listFinishedHats(),
            ])
        );
        exit;
    }
    ...
    $created = $repository->createFinishedHat($payload);

    \Forge\Server\ApiResponse::send(
        201,
        \Forge\Server\ApiResponse::success([
            'finished_hat' => $created,
        ])
    );
    ...
    forge_staff_log_unexpected_exception($exception, $bootstrapPath, 'staff catalog finished hats endpoint');
    forge_finished_hat_catalog_send_bootstrap_error('Finished hat catalog is currently unavailable.');
}
