...
function forge_finished_hat_catalog_send_bootstrap_error(string $message): void
{
    forge_staff_send_fallback_response(
        500,
        [
 ...
 * @param array<string, string> $fieldErrors
 */
function forge_finished_hat_catalog_send_validation_error(array $fieldErrors, string $message = 'Review the finished hat fields and try again.'): void
{
    http_response_code(422);
    header('Content-Type: application/json; charset=utf-8');
    header('Cache-Control: no-store');
...
function forge_finished_hat_catalog_resolve_upload_directory(): string
{
    return dirname(__DIR__, 4) . '/uploads/finished-hat-photos';
}
...
function forge_finished_hat_catalog_build_photo_relative_path(string $fileName): string
{
    return '/uploads/finished-hat-photos/' . ltrim($fileName, '/');
}
...
function forge_finished_hat_catalog_is_managed_photo_path(?string $photoPath): bool
{
    if (!is_string($photoPath)) {
        return false;
    }

    return str_starts_with($photoPath, '/uploads/finished-hat-photos/');
}
...
function forge_finished_hat_catalog_resolve_absolute_photo_path(?string $photoPath): ?string
{
    if (!forge_finished_hat_catalog_is_managed_photo_path($photoPath)) {
        return null;
    }

    $relativePath = substr($photoPath, strlen('/uploads/finished-hat-photos/'));
    if ($relativePath === false || $relativePath === '' || str_contains($relativePath, '..')) {
        return null;
    }

    return forge_finished_hat_catalog_resolve_upload_directory() . '/' . $relativePath;
}
